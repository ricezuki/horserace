...
DEFAULT_FLUSH_EVERY_RACES = 5
DEFAULT_RESTART_DRIVER_EVERY_RACES = 5
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape netkeiba odds by period using Selenium for all odds types."
    )
    ...
    parser.add_argument("--restart-driver-every-races", type=int, default=DEFAULT_RESTART_DRIVER_EVERY_RACES)
...
def recreate_driver(headless: bool, timeout_ms: int, old_driver=None):
    close_driver_quietly(old_driver)
    return create_selenium_driver(headless, timeout_ms)


def is_restartable_driver_error(exc: Exception) -> bool:
    if isinstance(exc, (InvalidSessionIdException, TimeoutException, WebDriverException, StaleElementReferenceException)):
        return True
    msg = str(exc)
    keywords = [
...
def scrape_b1_with_driver(driver, race_id: str, timeout_ms: int, field_size: Optional[int] = None) -> List[Dict]:
    driver.get(build_url("b1", race_id))
    ...
    tan_html = driver.find_element(By.CSS_SELECTOR, "#odds_tan_block table").get_attribute("outerHTML")
    fuku_html = driver.find_element(By.CSS_SELECTOR, "#odds_fuku_block table").get_attribute("outerHTML")
    ...
    df = pd.DataFrame(rows).drop_duplicates(subset=["race_id", "bet_type", "horse_number_1"], keep="first")
...
def dedupe_combo_rows(rows: List[Dict]) -> List[Dict]:
    if not rows:
        return rows
    df = pd.DataFrame(rows)
...
def selenium_parse_combo_page_simple(driver, race_id: str, odds_type: str, timeout_ms: int) -> List[Dict]:
    ...
    if rows and any(r.get("odds") is None for r in rows):
        ...
        selenium_wait_until_odds_nodes_ready(driver, odds_type, timeout_sec=max(20, int(timeout_ms / 1000)))
        rows = selenium_extract_combo_rows_current_view(driver, race_id, odds_type)
    return dedupe_combo_rows(rows)


def selenium_get_axis_select(driver):
    ...
    for sel in selects:
        try:
            ...
            parsed = []
            for o in options:
                value = clean_text(o.get_attribute("value") or "")
                text = clean_text(o.text)
                if re.match(r"^\d{1,2}\s", text) or re.match(r"^\d{1,2}$", value):
                    parsed.append((value, text))
            if len(parsed) >= 5:
                return sel
        except Exception:
            continue
    raise NoSuchElementException("axis select not found")


def selenium_get_select_options(driver) -> List[Dict[str, str]]:
    sel = selenium_get_axis_select(driver)
    options = sel.find_elements(By.TAG_NAME, "option")
    ...
    for o in options:
        value = clean_text(o.get_attribute("value") or "")
        text = clean_text(o.text)
        if not (re.match(r"^\d{1,2}\s", text) or re.match(r"^\d{1,2}$", value)):
            continue
...
def selenium_current_view_signature(driver, odds_type: str) -> str:
    odds_type_num = int(odds_type[1:])
    nodes = driver.find_elements(By.CSS_SELECTOR, f"[id^='odds-{odds_type_num}-']")[:20]
    parts = []
    for n in nodes:
        try:
            parts.append(f"{n.get_attribute('id') or ''}:{clean_text(n.text)}")
        except StaleElementReferenceException:
            continue
    return "|".join(parts)


def selenium_select_axis_option(driver, option_value: str) -> bool:
    sel = selenium_get_axis_select(driver)
    select = Select(sel)
    current = clean_text(select.first_selected_option.get_attribute("value") or "")
...
def selenium_wait_for_select_view_change(driver, odds_type: str, previous_signature: str, timeout_sec: int = 20) -> None:
    deadline = time.time() + timeout_sec
    while time.time() < deadline:
        sig = selenium_current_view_signature(driver, odds_type)
        if sig and sig != previous_signature:
            return
        time.sleep(0.3)


def selenium_parse_trifecta_like_page(driver, race_id: str, odds_type: str, timeout_ms: int) -> List[Dict]:
    selenium_wait_until_odds_nodes_ready(driver, odds_type, timeout_sec=max(20, int(timeout_ms / 1000)), min_ratio=0.95)
    collected: List[Dict] = []
    collected.extend(selenium_extract_combo_rows_current_view(driver, race_id, odds_type))

    options = selenium_get_select_options(driver)
    for idx, opt in enumerate(options, start=1):
        before_sig = selenium_current_view_signature(driver, odds_type)
        changed = selenium_select_axis_option(driver, opt["value"])
        if not changed:
            continue
        selenium_wait_for_select_view_change(driver, odds_type, before_sig, timeout_sec=max(20, int(timeout_ms / 1000)))
        selenium_wait_until_odds_nodes_ready(driver, odds_type, timeout_sec=max(20, int(timeout_ms / 1000)), min_ratio=0.95)
        view_rows = selenium_extract_combo_rows_current_view(driver, race_id, odds_type)
        none_count = sum(1 for r in view_rows if r.get("odds") is None)
        if view_rows and none_count > 0:
            time.sleep(1.2)
            selenium_wait_until_odds_nodes_ready(driver, odds_type, timeout_sec=max(20, int(timeout_ms / 1000)), min_ratio=0.95)
            view_rows = selenium_extract_combo_rows_current_view(driver, race_id, odds_type)
        collected.extend(view_rows)

    return dedupe_combo_rows(collected)


def scrape_single_odds_page(driver, race_id: str, odds_type: str, timeout_ms: int, field_size: Optional[int]) -> List[Dict]:
    driver.get(build_url(odds_type, race_id))
    if odds_type == "b1":
    ...
    if odds_type in {"b7", "b8"}:
        return selenium_parse_trifecta_like_page(driver, race_id, odds_type, timeout_ms)
    raise ValueError(f"unsupported odds_type={odds_type}")


def scrape_one_page_with_retry(
    selenium_driver,
    race_id: str,
    odds_type: str,
    timeout_ms: int,
    ...
    for attempt in range(1, max(1, int(args.retries)) + 1):
        try:
            rows = scrape_single_odds_page(driver, race_id, odds_type, timeout_ms, field_size)
            if odds_type == "b1":
                validate_b1_rows(rows, field_size=field_size)
            else:
            ...
            last_error = e
            if is_restartable_driver_error(e):
                print(f"  {odds_type}: restarting selenium driver after error={e}")
                driver = recreate_driver(args.headless, timeout_ms, driver)
            if attempt >= max(1, int(args.retries)):
                raise
            time.sleep((max(0, int(args.retry_backoff_ms)) * attempt) / 1000.0)
    raise last_error
...
def merge_odds_frames(existing_df: pd.DataFrame, new_rows: List[Dict]) -> pd.DataFrame:
    ...
    if existing_df is not None and not existing_df.empty:
        tmp = ensure_output_schema(existing_df.copy())
        if not tmp.empty:
            frames.append(tmp)
    if new_rows:
        tmp = ensure_output_schema(pd.DataFrame(new_rows))
...
def merge_error_frames(existing_df: pd.DataFrame, new_rows: List[Dict]) -> pd.DataFrame:
    ...
    df_err = df_err.drop_duplicates(subset=["race_id", "odds_type", "url", "error"], keep="last").reset_index(drop=True)
    return df_err
...
async def async_main(args: argparse.Namespace) -> None:
    ...
    restart_every_races = max(1, int(args.restart_driver_every_races))
    selenium_driver = create_selenium_driver(args.headless, args.timeout_ms)
    touched_since_restart = 0
    ...
    try:
        for i, race_id in enumerate(race_ids, start=1):
            ...
            for odds_type in args.types:
                ...
                touched_race = True
                try:
                    rows, selenium_driver = scrape_one_page_with_retry(
                    ...
                    if odds_type == "b1":
                        ...
                        print(f"  {odds_type} ({ODDS_TYPE_LABELS.get(odds_type, odds_type)}): tan={tan_cnt}, fuku={fuku_cnt}, active={active_tan}")
                    else:
                    ...
                    if is_restartable_driver_error(e):
                        selenium_driver = recreate_driver(args.headless, args.timeout_ms, selenium_driver)
            ...
            if touched_since_restart >= restart_every_races:
                existing_odds_df, existing_error_df = flush_buffers(
                    output_path=output_path,
                    error_path=error_path,
                    existing_odds_df=existing_odds_df,
                ...
                selenium_driver = recreate_driver(args.headless, args.timeout_ms, selenium_driver)
                touched_since_restart = 0
                print(f"  restarted selenium driver at race index {i}")
    finally:
        existing_odds_df, existing_error_df = flush_buffers(
            output_path=output_path,
            error_path=error_path,
        ...
        close_driver_quietly(selenium_driver)
