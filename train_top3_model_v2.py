...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train top3 prediction model with race-level evaluation."
    )
...
def compute_race_level_metrics(valid_out: pd.DataFrame) -> tuple[dict, pd.DataFrame]:
    ...
    for race_id, g in valid_out.groupby("race_id", sort=False):
        ...
        n = len(gg)
        actual_top3_total = int(gg["is_top3"].sum())

        def calc_at_k(k: int) -> dict:
            topk = gg.head(min(k, n)).copy()
            hits = int(topk["is_top3"].sum())

            hit_rate = float(hits > 0)
            precision = float(hits / len(topk)) if len(topk) > 0 else np.nan
            recall = float(hits / actual_top3_total) if actual_top3_total > 0 else np.nan
        ...
        rec = {
            ...
            "field_size": int(n),
            "actual_top3_total": actual_top3_total,
        }
    ...
    summary = {
        ...
        "top1_top3_hit_rate": float(race_metrics_df["top1_hit_rate"].mean()),
        "top1_top3_precision": float(race_metrics_df["top1_precision"].mean()),
        "top1_top3_recall": float(race_metrics_df["top1_recall"].mean(skipna=True)),
        "top3_top3_hit_rate": float(race_metrics_df["top3_hit_rate"].mean()),
        "top3_top3_precision": float(race_metrics_df["top3_precision"].mean()),
        "top3_top3_recall": float(race_metrics_df["top3_recall"].mean(skipna=True)),
        "top5_top3_hit_rate": float(race_metrics_df["top5_hit_rate"].mean()),
        "top5_top3_precision": float(race_metrics_df["top5_precision"].mean()),
        "top5_top3_recall": float(race_metrics_df["top5_recall"].mean(skipna=True)),
    }
...
def main() -> None:
    ...
    joblib.dump(model, output_dir / "top3_model.joblib")
