...
def get_place_horses_for_race(race_df: pd.DataFrame) -> list[int]:
    """
    今回のAI用ルール:
    複勝対象は常に「上位3頭」だけに固定する。
    同着があっても、着順順に並べて先頭3頭のみ採用。
    """
    df = race_df.dropna(subset=["finish_position_num", "horse_number"]).copy()
    df = df.sort_values(["finish_position_num", "horse_number"]).reset_index(drop=True)
    return df["horse_number"].astype(int).head(3).tolist()


def extract_place_df(payback_df: pd.DataFrame, race_result_df: pd.DataFrame) -> pd.DataFrame:
    place_df_raw = payback_df[payback_df["bet_type"].astype(str).str.strip() == "複勝"].copy()
    if place_df_raw.empty:
        return pd.DataFrame(
            columns=["race_id", "race_date", "horse_number", "place_hit", "place_payout_yen", "place_popularity"]
        )

    # payback 側の複勝は 1頭1行
    place_df_raw["horse_number"] = pd.to_numeric(place_df_raw["combination"], errors="coerce")
    place_df_raw["place_payout_yen"] = place_df_raw["payout_yen"].apply(parse_int_safe)
    place_df_raw["place_popularity"] = place_df_raw["popularity"].apply(parse_int_safe)

    place_df_raw = place_df_raw.dropna(subset=["horse_number", "place_payout_yen"]).copy()
    place_df_raw["horse_number"] = place_df_raw["horse_number"].astype(int)
    place_df_raw["place_hit"] = 1

    place_df_raw = place_df_raw[
        ["race_id", "race_date", "horse_number", "place_hit", "place_payout_yen", "place_popularity"]
    ].copy()

    # race_result 側の「今回採用する複勝対象3頭」だけを残す
    allowed_rows = []
    for race_id, g in race_result_df.groupby("race_id", sort=False):
        race_date = g["race_date"].iloc[0]
        top3_horses = get_place_horses_for_race(g)
        for horse_no in top3_horses:
            allowed_rows.append(
                {
                    "race_id": race_id,
                    "race_date": race_date,
...
def main() -> None:
    ...
    place_df = extract_place_df(payback_df, race_result_df)
