...
DEFAULT_RETRIES = 3
DEFAULT_SLEEP_MIN_MS = 800
DEFAULT_SLEEP_MAX_MS = 1800
DEFAULT_RETRY_BACKOFF_MS = 2500
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--retries", type=int, default=DEFAULT_RETRIES, help="Retry count for each race_id + odds_type")
    parser.add_argument(
        "--sleep-min-ms",
        type=int,
        default=DEFAULT_SLEEP_MIN_MS,
        help="Minimum sleep between requests in milliseconds",
    )
    parser.add_argument(
        "--sleep-max-ms",
        type=int,
        default=DEFAULT_SLEEP_MAX_MS,
        help="Maximum sleep between requests in milliseconds",
    )
    parser.add_argument(
        "--retry-backoff-ms",
        type=int,
        default=DEFAULT_RETRY_BACKOFF_MS,
        help="Base backoff milliseconds before retrying (multiplied by attempt number)",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
...
async def wait_for_page_ready(page, odds_type: str, timeout_ms: int) -> None:
    if odds_type == "b1":
        ...
        # 行数が途中段階のまま読まれないよう、単勝・複勝の行数が安定するまで待つ
        stable_needed = 3
        stable_count = 0
        last_tan = -1
        last_fuku = -1
        ...
        while True:
            ...
            () => {
              const tanRows = document.querySelectorAll('#odds_tan_block table tbody tr').length;
              const fukuRows = document.querySelectorAll('#odds_fuku_block table tbody tr').length;
              return { tanRows, fukuRows };
            }
            """)
            tan_rows = counts["tanRows"]
            fuku_rows = counts["fukuRows"]

            # 少なくとも9頭以上ないと、今回の「8頭で止まる」誤読を防げない
            enough_rows = tan_rows >= 9 and fuku_rows >= 9

            if tan_rows == last_tan and fuku_rows == last_fuku and enough_rows:
                stable_count += 1
            else:
                stable_count = 0
                last_tan = tan_rows
...
async def parse_b1_page(page, race_id: str, timeout_ms: int) -> List[Dict]:
    ...
    () => {
      ...
      function parsePopularity(text) {
        const m = normalize(text).match(/(\d+)人気/);
        return m ? parseInt(m[1], 10) : null;
      }

      function parseOdds(text, betType) {
        const s = normalize(text);
        if (!s || /^(取消|除外|中止|--+|---(?:\.-)?)$/.test(s)) {
          return { odds: null, odds_max: null };
        }
        ...
        if (betType === '複勝') {
          ...
          if (nums && nums.length >= 2) {
            return {
              odds: parseFloat(nums[0]),
              odds_max: parseFloat(nums[1])
            };
          }
      ...
      function getHorseNumberFromRow(tr) {
        // 馬番セルをクラスで優先取得
        const horseNumCell =
          tr.querySelector('td.Umaban') ||
          tr.querySelector('td.Horse_Num') ||
          tr.querySelector('td[class*="Umaban"]') ||
          tr.querySelector('td[class*="Horse_Num"]') ||
          tr.querySelector('td.Num.Txt_C');

        if (horseNumCell) {
          const t = txt(horseNumCell);
          if (/^\d{1,2}$/.test(t)) return parseInt(t, 10);
        }

        // フォールバック: 数字セル候補を複数拾い、枠番ではなく後ろ側を採用
        const numericCells = Array.from(tr.querySelectorAll('td, th'))
          .map(el => ({ text: txt(el), className: el.className || '' }))
          .filter(x => /^\d{1,2}$/.test(x.text));

        if (numericCells.length >= 2) {
          // 典型的には [枠番, 馬番]
          return parseInt(numericCells[1].text, 10);
        }
        if (numericCells.length === 1) {
          return parseInt(numericCells[0].text, 10);
        }

        return null;
      }

      function getHorseNameFromRow(tr) {
        const a =
          tr.querySelector('td.Horse_Info a') ||
          tr.querySelector('.Horse_Info a') ||
          tr.querySelector('a[title]') ||
          tr.querySelector('a');
        return txt(a);
      }

      function getOddsCellTextFromRow(tr) {
        // オッズ列をクラス優先で取る
        const oddsCell =
          tr.querySelector('td.Odds') ||
          tr.querySelector('td[class*="Odds"]');

        if (oddsCell) return txt(oddsCell);

        // フォールバック: 最後のセル
        const cells = Array.from(tr.querySelectorAll('th, td'));
        if (cells.length > 0) return txt(cells[cells.length - 1]);
      ...
      function parseTableRows(rootSelector, betType) {
        const out = [];
        const rows = Array.from(document.querySelectorAll(rootSelector + ' table tbody tr'));

        rows.forEach(tr => {
          const horseNo = getHorseNumberFromRow(tr);
          if (horseNo === null) return;

          const horseName = getHorseNameFromRow(tr);
          const oddsCellText = getOddsCellTextFromRow(tr);
          const parsedOdds = parseOdds(oddsCellText, betType);
          const popularity = parsePopularity(txt(tr));
