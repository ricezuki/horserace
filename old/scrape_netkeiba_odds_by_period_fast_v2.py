...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape netkeiba odds by period. b1 uses Selenium + pandas.read_html, others use Playwright."
    )
...
async def run_with_hard_timeout(coro, timeout_sec: float, label: str):
    try:
        return await asyncio.wait_for(coro, timeout=timeout_sec)
    except asyncio.TimeoutError:
        raise RuntimeError(f"{label}: hard timeout after {timeout_sec:.1f}s")

async def parse_trifecta_like_page(page, race_id: str, odds_type: str, timeout_ms: int) -> List[Dict]:
    base_timeout_sec = max(15, timeout_ms / 1000 + 5)

    print(f"    {odds_type}: initial load start")
    await run_with_hard_timeout(
        wait_until_odds_filled(page, odds_type, timeout_ms, min_ratio=0.95),
        base_timeout_sec,
        f"{odds_type} initial wait_until_odds_filled",
    )

    collected: List[Dict] = []

    first_rows = await run_with_hard_timeout(
        extract_combo_rows_current_view(page, race_id, odds_type),
        base_timeout_sec,
        f"{odds_type} initial extract",
    )
    collected.extend(first_rows)
    print(f"    {odds_type}: initial view rows={len(first_rows)}")

    options = await run_with_hard_timeout(
        get_select_options(page),
        base_timeout_sec,
        f"{odds_type} get_select_options",
    )
    print(f"    {odds_type}: select options={len(options)}")

    for idx, opt in enumerate(options, start=1):
        try:
            print(f"    {odds_type}: option {idx}/{len(options)} start value={opt['value']} text={opt['text']}")

            before_sig = await run_with_hard_timeout(
                current_view_signature(page, odds_type),
                base_timeout_sec,
                f"{odds_type} option {idx} current_view_signature",
            )

            changed = await run_with_hard_timeout(
                select_axis_option(page, opt["value"], opt["text"]),
                base_timeout_sec,
                f"{odds_type} option {idx} select_axis_option",
            )
            if not changed:
                print(f"    {odds_type}: option {idx}/{len(options)} skipped (not changed)")
                continue

            await run_with_hard_timeout(
                wait_for_select_view_change(page, odds_type, before_sig, timeout_ms),
                base_timeout_sec,
                f"{odds_type} option {idx} wait_for_select_view_change",
            )

            await run_with_hard_timeout(
                wait_until_odds_filled(page, odds_type, timeout_ms, min_ratio=0.95),
                base_timeout_sec,
                f"{odds_type} option {idx} wait_until_odds_filled",
            )

            view_rows = await run_with_hard_timeout(
                extract_combo_rows_current_view(page, race_id, odds_type),
                base_timeout_sec,
                f"{odds_type} option {idx} extract_combo_rows_current_view",
            )

            none_count = sum(1 for r in view_rows if r.get("odds") is None)
            if view_rows and none_count > 0:
                print(f"    {odds_type}: option {idx}/{len(options)} retry none_count={none_count}")
                await page.wait_for_timeout(1200)

                await run_with_hard_timeout(
                    wait_until_odds_filled(page, odds_type, timeout_ms, min_ratio=0.95),
                    base_timeout_sec,
                    f"{odds_type} option {idx} retry wait_until_odds_filled",
                )

                view_rows = await run_with_hard_timeout(
                    extract_combo_rows_current_view(page, race_id, odds_type),
                    base_timeout_sec,
                    f"{odds_type} option {idx} retry extract_combo_rows_current_view",
                )

            collected.extend(view_rows)
            print(f"    {odds_type}: option {idx}/{len(options)} rows={len(view_rows)} collected={len(collected)}")

        except Exception as e:
            print(f"    {odds_type}: option {idx}/{len(options)} error={e}")
            continue
    ...
    if collected:
        ...
        print(f"    {odds_type}: deduped rows={len(df)}")
        return df.to_dict("records")

    print(f"    {odds_type}: no rows collected")
    return collected
