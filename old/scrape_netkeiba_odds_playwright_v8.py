...
async def wait_for_page_ready(page, odds_type: str, timeout_ms: int) -> None:
    if odds_type == "b1":
        ...
        await page.wait_for_function(
            """
            () => {
              const tanRows = document.querySelectorAll('#odds_tan_block tr, #odds_tan_block tbody tr, .TanFukuTable:nth-of-type(1) tr').length;
              const fukuRows = document.querySelectorAll('#odds_fuku_block tr, #odds_fuku_block tbody tr, .TanFukuTable:nth-of-type(2) tr').length;
              return tanRows >= 2 || fukuRows >= 2;
            }
            """,
            timeout=timeout_ms,
        )
