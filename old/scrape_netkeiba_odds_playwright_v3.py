...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape netkeiba odds pages using Playwright-rendered DOM and direct getDataOdds switching."
    )
...
async def get_switch_params(page, odds_type: str) -> List[str]:
    """
    onclick="getDataOdds('b1','c1')" の第2引数を拾う。
    セレクタ依存ではなく、ページ内の getDataOdds 呼び出しを列挙する。
    """
    js = f"""
    () => {{
      const out = [];
...
async def extract_b1_rows_current_view(page, race_id: str) -> List[Dict]:
    ...
    () => {
      ...
      function parseRow(tr, betType) {
        const cells = Array.from(tr.querySelectorAll('th,td'));

        // b1 では「枠番」「馬番」の両方が数値列としてある前提
        const nums = [];
        ...
        if (odds === null) {
          const cellTexts = Array.from(tr.querySelectorAll('th,td,span')).map(x => txt(x));
          for (let i = cellTexts.length - 1; i >= 0; i--) {
            const s = (cellTexts[i] || '').replace(/,/g, '');
            if (/^\\d+(?:\\.\\d+)?$/.test(s)) {
              odds = parseFloat(s);
              break;
            }
    ...
    return [{**meta, "odds_type": "b1", **row} for row in parsed]
...
async def parse_b1_page(page, race_id: str, timeout_ms: int) -> List[Dict]:
    ...
    collected: List[Dict] = []
    collected.extend(await extract_b1_rows_current_view(page, race_id))

    params = await get_switch_params(page, "b1")

    # 初期表示も含め、切替キーを順に実行
    for key in params:
        try:
            await call_get_data_odds(page, "b1", key)
            await wait_for_page_ready(page, "b1", timeout_ms)
            collected.extend(await extract_b1_rows_current_view(page, race_id))
        except Exception:
            continue
