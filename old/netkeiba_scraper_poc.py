...
DISCOVERY_URLS = [
    ...
    BASE + "/top/payback_list.html?kaisai_date={yyyymmdd}",
    BASE + "/top/pdf.html?kaisai_date={yyyymmdd}",
]
...
def discover_race_ids_for_date(session: requests.Session, target_date: date, cfg: ScraperConfig) -> Set[str]:
    ...
    for tmpl in DISCOVERY_URLS:
        ...
        # hrefから抽出
        for race_id in set(re.findall(r"race_id=(\d{12})", html)):
            if race_id.startswith(target_date.strftime("%Y")):
                race_ids.add(race_id)

        # レース結果/出馬表の絶対URLや相対URLも拾う
...
def parse_conditions(race_data02: str) -> Dict[str, str]:
    ...
    if "G1" in race_data02 or "G2" in race_data02 or "G3" in race_data02:
        race_class = "重賞"
...
def fetch_race_result_html(session: requests.Session, race_id: str, cfg: ScraperConfig) -> Optional[str]:
    url = RESULT_URL.format(race_id=race_id)
    html = fetch_text(session, url, cfg)
    random_sleep(cfg)
    if not html:
        return None
    if "RaceTable01" not in html or "着 順" not in html:
        return None
    return html
...
def scrape_month(month: str, cfg: ScraperConfig) -> None:
    ...
    for d in daterange(start, end):
        ...
        if discovered:
            print(f"[DISCOVER] {d} -> {len(discovered)} race_ids")
        all_race_ids.update(discovered)

    all_race_ids = {rid for rid in all_race_ids if rid not in done_race_ids}
    print(f"[INFO] target month={month} discovered={len(all_race_ids)} new_races={len(all_race_ids)}")

    for race_id in sorted(all_race_ids):
        html = fetch_race_result_html(session, race_id, cfg)
        if not html:
            append_rows(
                log_csv,
                [{"race_id": race_id, "status": "failed_or_unavailable", "fetched_at": datetime.now().isoformat(timespec="seconds"), "html_sha1": ""}],
            )
            continue

        save_raw_html(cfg, race_id, html)
        soup = BeautifulSoup(html, "html.parser")
        ...
        if not result_rows:
            append_rows(
                log_csv,
                [{"race_id": race_id, "status": "parsed_no_rows", "fetched_at": datetime.now().isoformat(timespec="seconds"), "html_sha1": html_sha1(html)}],
            )
            continue
        ...
        append_rows(
            log_csv,
            [{"race_id": race_id, "status": "ok", "fetched_at": datetime.now().isoformat(timespec="seconds"), "html_sha1": html_sha1(html)}],
        )
        print(f"[OK] {race_id} rows={len(result_rows)} paybacks={len(payback_rows)}")


def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="netkeiba monthly scraper PoC")
    parser.add_argument("--month", required=True, help="target month in YYYY-MM format")
    parser.add_argument("--output-dir", default="./data/netkeiba_poc", help="output directory")
