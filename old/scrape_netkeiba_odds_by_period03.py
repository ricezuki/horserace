...
async def wait_for_odds_population(
    page,
    odds_type: str,
    timeout_ms: int,
    ...
    while True:
        ...
        if (filled, total) > (best["filled"], best["total"]):
            best = {"total": total, "filled": filled}

        key = (total, filled)
        fill_ratio = (filled / total) if total > 0 else 0.0
        if key == prev_key and total > 0 and fill_ratio >= min_fill_ratio:
            stable += 1
        else:
            stable = 0
            prev_key = key

        if total > 0 and fill_ratio >= min_fill_ratio and stable >= stable_cycles:
...
def count_missing_odds(rows: List[Dict]) -> int:
    return sum(1 for row in rows if row.get("odds") is None)


async def extract_combo_rows_stable(
    page,
    race_id: str,
    odds_type: str,
    timeout_ms: int,
    retry_if_missing: int = 2,
) -> List[Dict]:
    await wait_for_page_ready(page, odds_type, timeout_ms)
    await wait_for_odds_population(page, odds_type, timeout_ms)

    rows = await extract_combo_rows_current_view(page, race_id, odds_type)
    missing = count_missing_odds(rows)

    attempts = 0
    while rows and missing > 0 and attempts < retry_if_missing:
        await page.wait_for_timeout(800 + 400 * attempts)
        await wait_for_odds_population(page, odds_type, timeout_ms)
        candidate_rows = await extract_combo_rows_current_view(page, race_id, odds_type)
        candidate_missing = count_missing_odds(candidate_rows)

        if len(candidate_rows) >= len(rows) and candidate_missing <= missing:
            rows = candidate_rows
...
async def parse_combo_page_simple(page, race_id: str, odds_type: str, timeout_ms: int) -> List[Dict]:
    rows = await extract_combo_rows_stable(page, race_id, odds_type, timeout_ms)
...
async def parse_trifecta_like_page(page, race_id: str, odds_type: str, timeout_ms: int) -> List[Dict]:
    ...
    collected: List[Dict] = []
    seen_axis_values = set()

    # 初期表示も回収
    base_rows = await extract_combo_rows_stable(page, race_id, odds_type, timeout_ms, retry_if_missing=3)
    collected.extend(base_rows)

    options = await get_select_options(page)

    for opt in options:
        axis_value = opt["value"]
        if axis_value in seen_axis_values:
            continue
        seen_axis_values.add(axis_value)
        ...
        try:
            ...
            axis_rows = await extract_combo_rows_stable(page, race_id, odds_type, timeout_ms, retry_if_missing=3)
            collected.extend(axis_rows)
        except Exception:
            continue
    ...
    if collected:
        ...
        # まだ欠損が残る行だけ最後にまとめて再試行
        rows = df.to_dict("records")
        if count_missing_odds(rows) > 0:
            await page.wait_for_timeout(1200)
            retry_rows = await extract_combo_rows_stable(page, race_id, odds_type, timeout_ms, retry_if_missing=4)
            if retry_rows:
        ...
        for i, race_id in enumerate(race_ids, start=1):
            ...
            for odds_type in args.types:
                ...
                try:
                    ...
                    missing = count_missing_odds(rows)
                    if missing > 0 and odds_type != "b1":
                        print(f"  {odds_type} ({ODDS_TYPE_LABELS.get(odds_type, odds_type)}): {len(rows)} rows / missing_odds={missing}")
                    else:
