...
# -*- coding: utf-8 -*-
"""
netkeiba scrape + parse unified script
...
Examples
  python netkeiba_scrape_and_parse.py --period 2025 --output-dir ./data/netkeiba_2025 --save-html
  python netkeiba_scrape_and_parse.py --period 2026-01 --output-dir ./data/netkeiba_2026_01 --save-html
  python netkeiba_scrape_and_parse.py --period 2026-01 --output-dir ./data/netkeiba_2026_01 --parse-only
"""
...
def parse_grade(soup: BeautifulSoup, race_name: str, race_data02_raw: str) -> str:
    candidates = []
    for selector in [
        ...
        if el is None:
            continue
        if el.name == 'meta':
            candidates.append(clean_text(el.get('content', '')))
        else:
            candidates.append(clean_text(el.get_text(' ', strip=True)))
    candidates.append(clean_text(race_name))
    candidates.append(clean_text(race_data02_raw))

    joined = ' | '.join(candidates)
    m = re.search(r'\((G1|G2|G3|L)\)', joined)
    if m:
        return m.group(1)
    if 'リステッド' in joined:
        return 'L'

    # fallback to icon class when text omits explicit grade
    h1 = soup.select_one('.RaceName')
    if h1:
        classes = []
        for sp in h1.select('span.Icon_GradeType'):
            classes.extend(sp.get('class', []))
        cls_text = ' '.join(classes)
        if 'Icon_GradeType1' in cls_text:
            return 'G1'
        if 'Icon_GradeType2' in cls_text:
            return 'G2'
        if 'Icon_GradeType3' in cls_text:
            return 'G3'
        if 'Icon_GradeType4' in cls_text:
            return 'L'
    return ''

def parse_race_class(race_data02_raw: str, race_name: str, grade: str = '') -> str:
    src = normalize_zen_digits(f"{race_name} {race_data02_raw}")
    if grade in {'G1', 'G2', 'G3', 'L'}:
        return 'オープン'
...
def parse_race_meta(soup: BeautifulSoup, race_id: str) -> Dict[str, str]:
    ...
    place_code = race_id[4:6]
    grade = parse_grade(soup, race_name, rd2)
    race_class = parse_race_class(rd2, race_name, grade)
    return {
        "race_id": race_id,
        "race_date": race_date,
        "year": race_id[:4],
...
def main() -> None:
    ...
    for month_start, month_end, month_label in month_ranges:
        ...
        if cfg.parse_only:
            race_ids = sorted({
                m.group(1) for path in cfg.html_dir.glob("*.html")
