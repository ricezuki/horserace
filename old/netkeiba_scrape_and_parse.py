...
def parse_race_meta(soup: BeautifulSoup, race_id: str) -> Dict[str, str]:
    ...
    return {
        ...
        "race_name": race_name,
        "grade": race_class if race_class in {"G1", "G2", "G3", "L"} else "",
...
def main() -> None:
    ...
    start, end, label = parse_period(cfg.period)
    ...
    if cfg.parse_only:
        race_ids = sorted({m.group(1) for path in cfg.html_dir.glob("*.html") if (m := re.search(r"(\d{12})", path.name))})
    else:
        race_ids = sorted(discover_all_race_ids(session, start, end, cfg))
