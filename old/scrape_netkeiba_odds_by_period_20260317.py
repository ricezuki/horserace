...
def load_race_input(path: str, period: str) -> Tuple[List[str], Dict[str, Optional[int]]]:
    ...
    return race_ids, field_size_map
        ...
        for i, race_id in enumerate(race_ids, start=1):
            ...
            for odds_type in args.types:
                ...
                try:
                    ...
                    if odds_type == "b1":
                        ...
                        print(f"  {odds_type} ({ODDS_TYPE_LABELS.get(odds_type, odds_type)}): tan={tan_cnt}, fuku={fuku_cnt}")
                    else:
                        expected = expected_count_for_odds_type(odds_type, field_size)
                        if expected is None:
            ...
            if error_rows_buffer:
                ...
                if error_path.exists():
                    ...
                    if not df_err_old.empty:
                        err_frames.append(df_err_old)
                if not df_err_new.empty:
                    err_frames.append(df_err_new)
                ...
                if err_frames:
                    ...
                    df_err = pd.DataFrame(columns=["race_id", "odds_type", "url", "error"])
